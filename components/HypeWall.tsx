    ...
    return (
        <div className="space-y-12">
            <div className="flex items-center justify-between">
                <h2 className="text-4xl font-bold tracking-tighter uppercase">Community Feed</h2>
                <span className="text-[10px] font-bold opacity-40 uppercase tracking-[0.3em]">Verified Holders Only</span>
            </div>

            <div className="border-t-2 border-black">
                {messages.map((m) => (
                    <div key={m.id} className="py-6 border-b border-black/10 flex justify-between items-start gap-10">
                        <span className="text-xl font-bold tracking-tight">{m.text}</span>
                        <span className="text-[10px] font-bold opacity-40 uppercase tracking-widest pt-2 shrink-0">{m.author}</span>
                    </div>
                ))}
            </div>
