...
export default function MilestoneCard({ milestone, unlocked, idx }: MilestoneCardProps) {
    return (
        <motion.div
            ...
            className={`p-6 md:p-10 transition-all duration-500 relative overflow-hidden flex flex-col justify-center min-h-[160px] md:min-h-[200px] border-r border-b border-black ${unlocked ? 'bg-white text-black' : 'bg-black text-white'}`}
        >
            {/* Status Badge */}
            ...
            <div className="space-y-4">
                <div className="flex items-center gap-4">
                    {unlocked && (
                        <div className="w-8 h-8 md:w-10 md:h-10 flex items-center justify-center border-2 border-black rounded-lg shrink-0">
                            {milestone.icon}
                        </div>
                    )}
                ...
                {unlocked ? (
                    <p className="text-sm md:text-base font-medium opacity-60 max-w-md italic pt-4 border-t border-black/10">
                        {milestone.description}
                    </p>
                ) : (
                    <div className="h-4 w-32 bg-white/20 animate-pulse mt-4 border-t border-white/10" />
                )}
            </div>
        </motion.div>
    );
